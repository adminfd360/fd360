...
import banner from './assets/tempaltes-banner.jpg'
  ...
  return (
    <>
      <div className="h-[380px] md:h-[480px] w-full bg-cover bg-center bg-no-repeat flex items-center justify-center"
        ...
        <p className="text-white text-[clamp(30px,3.2vw,55px)] font-semibold">Website Templates</p>
      </div>
      <div className="w-full">
        <div className="w-full max-w-[1280px] p-4 py-20 mx-auto">
          <h1 className="text-2xl text-center font-semibold">Choose from our templates</h1>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 mt-12">
            {templateItems.map((item, index) => (
              <div key={index} className="shadow-xl rounded-lg relative overflow-hidden group">
                <img className="group-hover:scale-105 transition-all duration-1000" src={item.image} alt="" />
                <div className="transition-all duration-500 opacity-0 group-hover:opacity-100">
                  <div className="w-[400px] h-[400px] bg-[#000] rounded-full absolute -bottom-[400px] -left-[400px] block opacity-50 transition-all duration-700 group-hover:-bottom-20 group-hover:-left-20">
                  </div>
                  <div className="w-[350px] h-[350px] bg-gradient-to-t from-[#252525] to-[#000] opacity-80 rounded-full absolute -bottom-[400px] -left-[400px] transition-all duration-300 group-hover:-bottom-16 group-hover:-left-16 block">
                  </div>
                  <div className="absolute bottom-0 left-0 p-6 pb-10">
                    <p className="text-3xl mb-4 text-white font-bold">{item.title}</p>
                    <Link to={item.link} target="_blank" className="rounded-full bg-white p-4 inline-block hover:bg-sky-700 hover:text-white">Check Template</Link>
                  </div>
                </div>
              </div>
            ))}
